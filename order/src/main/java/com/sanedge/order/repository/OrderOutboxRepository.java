...
@ApplicationScoped
public class OrderOutboxRepository implements PanacheRepository<OrderOutbox> {
    ...
    @WithTransaction
    public Uni<List<OrderOutbox>> findDue(int limit) {
    ...
    @WithTransaction
    public Uni<OrderOutbox> claim(OrderOutbox event) {
        Timestamp now = Timestamp.valueOf(LocalDateTime.now());
        Timestamp leaseCutoff = Timestamp.valueOf(LocalDateTime.now().minusSeconds(300));
    ...
    @WithTransaction
    public Uni<Void> markSent(OrderOutbox event) {
    ...
    @WithTransaction
    public Uni<Void> markRetry(OrderOutbox event, Throwable failure, long delaySeconds) {
