...
@ApplicationScoped
public class ProductOutboxRepository implements PanacheRepository<ProductOutbox> {
    ...
    @WithTransaction
    public Uni<List<ProductOutbox>> findDue(int limit) {
    ...
    @WithTransaction
    public Uni<ProductOutbox> claim(ProductOutbox event) {
        Timestamp now = Timestamp.valueOf(LocalDateTime.now());
        Timestamp leaseCutoff = Timestamp.valueOf(LocalDateTime.now().minusSeconds(300));
    ...
    @WithTransaction
    public Uni<Void> markSent(ProductOutbox event) {
    ...
    @WithTransaction
    public Uni<Void> markRetry(ProductOutbox event, Throwable failure, long delaySeconds) {
