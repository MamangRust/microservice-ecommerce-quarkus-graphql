...
import pb.category.stats.MutinyCategoryPriceByIdServiceGrpc;

@GrpcService
@Singleton
public class CategoryPriceByIdGrpcHandler
        extends MutinyCategoryPriceByIdServiceGrpc.CategoryPriceByIdServiceImplBase {

    @Inject
    ClickHouseStatsRepository repo;

    @Override
    public Uni<CategoryCommon.ApiResponseCategoryMonthPrice> findMonthPriceById(CategoryCommon.FindYearCategoryById request) {
        return repo.findMonthlyCategoryPriceById(request.getYear(), request.getCategoryId())
                .map(rows -> {
                    var b = CategoryCommon.ApiResponseCategoryMonthPrice.newBuilder()
                            .setStatus("success").setMessage("Retrieved monthly category prices by ID");
                    for (Object o : rows) {
    ...
    @Override
    public Uni<CategoryCommon.ApiResponseCategoryYearPrice> findYearPriceById(CategoryCommon.FindYearCategoryById request) {
        return repo.findYearlyCategoryPriceById(request.getYear(), request.getCategoryId())
                .map(rows -> {
                    var b = CategoryCommon.ApiResponseCategoryYearPrice.newBuilder()
                            .setStatus("success").setMessage("Retrieved yearly category prices by ID");
                    for (Object o : rows) {
                        JsonObject r = (JsonObject) o;
