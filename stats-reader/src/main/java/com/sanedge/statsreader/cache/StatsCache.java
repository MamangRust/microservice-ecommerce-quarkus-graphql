 ...
 *
 * Every key is namespaced under {@code apigw:stats:} — the gateway-facing
 * stats cache must be separated from the domain service caches. Cache reads
 * are fail-open: if Redis is down the ClickHouse query still runs, so a
 * cache outage never takes the stats API down.
 */
@ApplicationScoped
public class StatsCache {
    ...
    private LongCounter cacheCounter;

    @PostConstruct
    void initMetrics() {
        Meter meter = openTelemetry.getMeter("stats-reader");
        this.cacheCounter = meter.counterBuilder("cache_outcome_total")
                .setDescription("Cache outcome counts (hit, miss, fallback)")
                .build();
    }

    private void recordHit() {
        if (cacheCounter != null) cacheCounter.add(1, Attributes.of(RESULT_KEY, "hit"));
    }

    private void recordMiss() {
        if (cacheCounter != null) cacheCounter.add(1, Attributes.of(RESULT_KEY, "miss"));
    }

    private void recordFallback() {
        if (cacheCounter != null) cacheCounter.add(1, Attributes.of(RESULT_KEY, "fallback"));
    }
    ...
    public Uni<JsonArray> cacheArray(String key, Supplier<Uni<JsonArray>> loader) {
        ...
        return redisService.getReactive(cacheKey)
                ...
                .onFailure().recoverWithUni(err -> {
                    recordFallback();
                    log.warn("Stats cache read failed for {}: {} — falling back to ClickHouse",
                            key, err.getMessage());
                    return loader.get();
                });
    }
    ...
    /**
     * Caches a JsonObject result (dashboard-style responses).
     */
    ...
    /** SHA-1 hex of the SQL query — stable cache key for a given SQL. */
