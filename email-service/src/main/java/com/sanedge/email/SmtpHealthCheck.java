...
/**
 * Dependency-aware readiness check for SMTP. Performs a real protocol-level
 * round trip (greeting + {@code NOOP}) against the configured mailer host, so
 * readiness reflects actual SMTP reachability rather than config presence.
 */
@Readiness
@ApplicationScoped
...
public class SmtpHealthCheck implements HealthCheck {
    ...
    public HealthCheckResponse call() {
        ...
        try (Socket socket = new Socket()) {
            ...
            // RFC 5321: EHLO must precede any other command. Some strict servers
            // reject NOOP before EHLO, so use EHLO (multi-line 250-... accepted).
